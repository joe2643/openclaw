...
const bindingServiceMocks = vi.hoisted(() => ({
  ...
  unbind: vi.fn<
    (input: { targetSessionKey?: string; reason: string }) => Promise<SessionBindingRecord[]>
  >(async () => []),
}));
  ...
  beforeEach(async () => {
    ...
    vi.doMock("../../infra/outbound/session-binding-service.js", () => ({
      getSessionBindingService: () => ({
        ...
        unbind: (input: { targetSessionKey?: string; reason: string }) =>
          bindingServiceMocks.unbind(input),
      }),
    }));
